...
import { VendorPowService } from '../services/vendor-pow.service';
...
export const vendorSecurityInterceptor: HttpInterceptorFn = (req, next) => {
  const tokenService = inject(VendorAuthTokenService);
  const powService = inject(VendorPowService);
